...
/**
 * Add the meta options for the Grid Prices to the DownloadGrid Metabox
 *
 * @access   public
 * @since    1.0.0
 * @param    Void
 ...
 */
function mp_stacks_downloadgrid_price_meta_options( $items_array ){		
...
add_filter( 'mp_stacks_downloadgrid_items_array', 'mp_stacks_downloadgrid_price_meta_options', 12 );
 ...
 * @return   Array - All of the placement optons needed for Price
 */
function mp_stacks_downloadgrid_price_placement_options( $placement_options, $post_id ){
	
	//Show Post Prices
	$placement_options['price_show'] = mp_core_get_post_meta($post_id, 'downloadgrid_price_show');

	//Prices Placement
	$placement_options['price_placement'] = mp_core_get_post_meta($post_id, 'downloadgrid_price_placement', 'over_image_top_left');
	
	return $placement_options;	
}
add_filter( 'mp_stacks_downloadgrid_placement_options', 'mp_stacks_downloadgrid_price_placement_options', 10, 2 );
...
function mp_stacks_downloadgrid_price( $post_id ){
	...
	$downloadgrid_output = mp_stacks_grid_highlight_text_html( array( 
		...
		'output_string' => apply_filters( 'mp_stacks_downloadgrid_item_price', $the_price, $post_id ), 
	) );
 ...
 */
function mp_stacks_downloadgrid_price_top_over_callback( $downloadgrid_output, $grid_post_id, $options ){
	...
	if ( strpos( $options['price_placement'], 'over') !== false && strpos( $options['price_placement'], 'top') !== false && $options['price_show']){
		...
		return $downloadgrid_output . mp_stacks_downloadgrid_price( $grid_post_id, $options['word_limit'], $options['read_more_text'] );
...
add_filter( 'mp_stacks_downloadgrid_top_over', 'mp_stacks_downloadgrid_price_top_over_callback', 10, 3 );
 ...
 * @return   $html_output String - A string holding the html for text over a featured image in the grid
 */
function mp_stacks_downloadgrid_price_middle_over_callback( $downloadgrid_output, $grid_post_id, $options ){
...
add_filter( 'mp_stacks_downloadgrid_middle_over', 'mp_stacks_downloadgrid_price_middle_over_callback', 10, 3 );
 ...
 * @return   $html_output String - A string holding the html for text over a featured image in the grid
 */
function mp_stacks_downloadgrid_price_bottom_over_callback( $downloadgrid_output, $grid_post_id, $options ){
...
add_filter( 'mp_stacks_downloadgrid_bottom_over', 'mp_stacks_downloadgrid_price_bottom_over_callback', 10, 3 );
 ...
 * @return   $html_output String - A string holding the html for text over a featured image in the grid
 */
function mp_stacks_downloadgrid_price_below_over_callback( $downloadgrid_output, $grid_post_id, $brick_id, $options ){
	
	//If we should show the price below the image
	if ( strpos( $options['price_placement'], 'below') !== false && $options['price_show']){
		
		$price_html_output = '<a href="' . apply_filters( 'mp_stacks_downloadgrid_grid_post_permalink', get_permalink(), $grid_post_id, $brick_id ) . '" class="mp-stacks-downloadgrid-price-link">';	
			$price_html_output .= mp_stacks_downloadgrid_price( $grid_post_id, $options['word_limit'], $options['read_more_text'] );
		$price_html_output .= '</a>';
		
		return $downloadgrid_output . $price_html_output;
	}
	
	return $downloadgrid_output;
	
}
add_filter( 'mp_stacks_downloadgrid_below', 'mp_stacks_downloadgrid_price_below_over_callback', 10, 4 );

/**
 * Add the JS for the price to DownloadGrid's HTML output
 *
 * @access   public
 * @since    1.0.0
 ...
 */
function mp_stacks_downloadgrid_price_animation_js( $existing_filter_output, $post_id, $meta_prefix ){
	...
	$price_animation_js = mp_core_js_mouse_over_animate_child( '#mp-brick-' . $post_id . ' .mp-stacks-grid-item', '.mp-stacks-downloadgrid-item-price-holder', mp_core_get_post_meta( $post_id, 'downloadgrid_price_animation_keyframes', array() ), true, true, 'mp-brick-' . $post_id ); 

	return $existing_filter_output . $price_animation_js;
}
add_filter( 'mp_stacks_grid_js', 'mp_stacks_downloadgrid_price_animation_js', 10, 3 );
		
/**
 * Add the CSS for the price to DownloadGrid's CSS
 *
 * @access   public
 * @since    1.0.0
