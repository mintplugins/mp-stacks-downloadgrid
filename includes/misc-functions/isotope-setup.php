 ...
 *
 * To use for your own Add-On, find and replace "downloadgrid" with your plugin's prefix
 *
 * @since 1.0.0
 *
...
/**
 * Add the meta options for "Isotope" to the DownloadGrid Metabox
 *
 * @access   public
 * @since    1.0.0
 * @param    Void
...
*/
function mp_stacks_downloadgrid_add_isotope_meta( $items_array ){
...
add_filter( 'mp_stacks_' . 'downloadgrid' . '_items_array', 'mp_stacks_downloadgrid_add_isotope_meta', 14 );
...
*/
function mp_stacks_downloadgrid_isotope_filter_group_options( $isotope_filter_groups ){
	
	//This array can contain custom groups (for outside sources like instgram), AND/OR WordPress taxonomy slugs.			
	$isotope_filter_groups = mp_stacks_downloadgrid_isotope_filter_groups();
...
add_filter( 'downloadgrid' . '_isotope_filter_groups', 'mp_stacks_downloadgrid_isotope_filter_group_options' );
 ...
 * @return   Array - Returning out it is an array containing the Filter Groups that the user can choose from.
*/
function mp_stacks_downloadgrid_isotope_filter_groups(){
	
	//This array can contain custom groups (for outside sources like instgram), AND/OR WordPress taxonomy slugs.			
	$isotope_filter_groups = array( 
		'download_category' => array( 
			'is_wordpress_taxonomy' => true,
			'filter_group_name' => __( 'Categories', 'mp_stacks_downloadgrid' ),
		...
		'download_tag' => array(
			'is_wordpress_taxonomy' => true,
			'filter_group_name' => __( 'Tags', 'mp_stacks_downloadgrid' ),
			'meta_field_ids_representing_tax_term' => array(
				'taxonomy_term' => array()
			),
			//Icon info
...
*/
function mp_stacks_downloadgrid_all_icon( $isotope_icon, $meta_prefix ){
