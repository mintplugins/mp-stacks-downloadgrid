 ...
 * @return   $downloadgrid_output String - The incoming HTML with the new JS animation for the excerpt appended.
 */
function mp_stacks_downloadgrid_excerpt_animation_js( $downloadgrid_output, $post_id ){
	
	//Get JS output to animate the excerpts on mouse over and out
	$excerpt_animation_js = mp_core_js_mouse_over_animate_child( '#mp-brick-' . $post_id . ' .mp-stacks-downloadgrid-item', '.mp-stacks-downloadgrid-item-excerpt-holder', mp_core_get_post_meta( $post_id, 'downloadgrid_excerpt_animation_keyframes', array() ) ); 

	return $downloadgrid_output . $excerpt_animation_js;
}
add_filter( 'mp_stacks_downloadgrid_animation_js', 'mp_stacks_downloadgrid_excerpt_animation_js', 10, 2 );
