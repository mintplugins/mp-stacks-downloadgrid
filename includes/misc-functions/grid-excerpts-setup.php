...
/**
 * Add the meta options for the Grid Excerpts to the DownloadGrid Metabox
 *
 * @access   public
 * @since    1.0.0
 * @param    Void
...
function mp_stacks_downloadgrid_excerpt_meta_options( $items_array ){		
	...
	$new_fields = array(
		...
		'downloadgrid_excerpt_google_font' => array(
			'field_id'			=> 'downloadgrid_excerpt_google_font',
		...
		'downloadgrid_excerpt_google_font_weight_style' => array(
			'field_id'			=> 'downloadgrid_excerpt_google_font_weight_style',
		...
		'downloadgrid_excerpt_spacing' => array(
			'field_id'			=> 'downloadgrid_excerpt_spacing',
			'field_title' 	=> __( 'Excerpts\' Spacing', 'mp_stacks_postgrid'),
			'field_description' 	=> __( 'How much space should there be between the excerpt and anything directly above it? Default: 10', 'mp_stacks_postgrid' ),
			'field_type' 	=> 'number',
			'field_value' => '10',
			'field_showhider' => 'downloadgrid_excerpt_settings',
		),
...
add_filter( 'mp_stacks_downloadgrid_items_array', 'mp_stacks_downloadgrid_excerpt_meta_options', 11 );
 ...
 * @return   Array - All of the placement optons needed for Excerpt
 */
function mp_stacks_downloadgrid_excerpt_placement_options( $placement_options, $post_id ){
	
	//Show Post Excerpts
	$placement_options['excerpt_show'] = mp_core_get_post_meta($post_id, 'downloadgrid_excerpt_show');

	//Excerpts Placement
	$placement_options['excerpt_placement'] = mp_core_get_post_meta($post_id, 'downloadgrid_excerpt_placement', 'below_image_left');
...
add_filter( 'mp_stacks_downloadgrid_placement_options', 'mp_stacks_downloadgrid_excerpt_placement_options', 10, 2 );
 ...
 */
function mp_stacks_downloadgrid_excerpt_top_over_callback( $downloadgrid_output, $grid_post_id, $options ){
	...
	if ( strpos( $options['excerpt_placement'], 'over') !== false && strpos( $options['excerpt_placement'], 'top') !== false && $options['excerpt_show']){
		...
		return $downloadgrid_output . mp_stacks_downloadgrid_excerpt( $grid_post_id, $options['word_limit'], $options['read_more_text'] );
...
add_filter( 'mp_stacks_downloadgrid_top_over', 'mp_stacks_downloadgrid_excerpt_top_over_callback', 10, 3 );
 ...
 * @return   $html_output String - A string holding the html for text over a featured image in the grid
 */
function mp_stacks_downloadgrid_excerpt_middle_over_callback( $downloadgrid_output, $grid_post_id, $options ){
...
add_filter( 'mp_stacks_downloadgrid_middle_over', 'mp_stacks_downloadgrid_excerpt_middle_over_callback', 10, 3 );
 ...
 * @return   $html_output String - A string holding the html for text over a featured image in the grid
 */
function mp_stacks_downloadgrid_excerpt_bottom_over_callback( $downloadgrid_output, $grid_post_id, $options ){
...
add_filter( 'mp_stacks_downloadgrid_bottom_over', 'mp_stacks_downloadgrid_excerpt_bottom_over_callback', 10, 3 );
...
function mp_stacks_downloadgrid_excerpt_below_over_callback( $downloadgrid_output, $grid_post_id, $post_id, $options ){
	...
	if ( strpos( $options['excerpt_placement'], 'below') !== false && $options['excerpt_show']){
		...
		return $downloadgrid_output . $excerpt_html_output;
	}
 ...
 */
function mp_stacks_downloadgrid_excerpt_animation_js( $existing_filter_output, $post_id, $meta_prefix ){
	
	if ( $meta_prefix != 'downloadgrid' ){
...
add_filter( 'mp_stacks_grid_js', 'mp_stacks_downloadgrid_excerpt_animation_js', 10, 3 );
		
/**
 * Add the CSS for the excerpt to DownloadGrid's CSS
 *
 * @access   public
 * @since    1.0.0
 ...
 */
function mp_stacks_downloadgrid_excerpt_css( $css_output, $post_id ){
	...
	return $css_output .= mp_stacks_grid_text_css( $post_id, 'downloadgrid_excerpt', 'mp-stacks-downloadgrid-item-excerpt', $excerpt_css_defaults );
}
 ...
 */
function mp_stacks_downloadgrid_excerpt_google_font( $css_output, $post_id, $first_content_type, $second_content_type ){
	...
	$downloadgrid_excerpt_googlefont = mp_core_get_post_meta( $post_id, 'downloadgrid_excerpt_google_font' );
	$downloadgrid_excerpt_googlefontweight = mp_core_get_post_meta( $post_id, 'downloadgrid_excerpt_google_font_weight_style' );
	
	//If a font name has been entered
	if ( !empty( $downloadgrid_excerpt_googlefont ) ){
		
		//Check if a font extra (weight) has been selected and add it if so.
		$downloadgrid_excerpt_googlefontweight = isset($downloadgrid_excerpt_googlefontweight) && !empty( $downloadgrid_excerpt_googlefontweight ) ? ':' . $downloadgrid_excerpt_googlefontweight : NULL;
		$downloadgrid_excerpt_googlefont = $downloadgrid_excerpt_googlefont . $downloadgrid_excerpt_googlefontweight;
	
		//Load the Google Font using the Google Font Class in MP Core
		new MP_CORE_Font( $downloadgrid_excerpt_googlefont, $downloadgrid_excerpt_googlefont, $mp_core_google_font_args );
		$mp_stacks_footer_inline_css[$downloadgrid_excerpt_googlefont] = $mp_core_font_families[$downloadgrid_excerpt_googlefont];
		
		//Return the incoming css string plus css to apply this font family to all paragraph tags
		$css_output .=  '#mp-brick-' . $post_id . ' .mp-stacks-downloadgrid-item-excerpt, #mp-brick-' . $post_id . ' .mp-stacks-downloadgrid-item-excerpt * { font-family: \'' . $downloadgrid_excerpt_googlefont . '\';}';
	
	}
	
	return $css_output;	
}
add_filter('mp_brick_additional_css', 'mp_stacks_downloadgrid_excerpt_google_font', 10, 4);	